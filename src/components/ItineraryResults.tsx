      ...
      <div className="p-6">
        ...
        <div className="space-y-6">
          {itinerary.days.map((day, index) => (
            <div key={day.day} className="bg-gray-50 rounded-lg p-5 relative">
              {/* Copy button */}
              <button
                onClick={() => copyDayToClipboard(day, day.day)}
                className="absolute top-3 right-3 p-2 text-gray-400 hover:text-gray-600 hover:bg-white rounded-md transition-colors"
              ...
              <div className="mb-4">
                ...
                <ul className="space-y-2">
                  {day.activities.map((activity, actIndex) => (
                    <li key={actIndex} className="text-gray-700 pl-4 relative">
              ...
              {day.notes && (
                <div className="pt-3 border-t border-gray-200">
                  <p className="text-gray-600 text-sm italic">{day.notes}</p>
                </div>
              )}
            </div>
          ))}
          ...
          <ul className="space-y-2">
            {itinerary.tips.map((tip, index) => (
              <li key={index} className="text-gray-700 flex items-start gap-2">
                <span className="text-blue-600 mt-1.5 w-2 h-2 bg-blue-600 rounded-full flex-shrink-0"></span>
                {tip}
              </li>
            ))}
          </ul>
